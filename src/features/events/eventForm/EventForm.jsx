...
const EventForm = ({ match, history, location }) => {
    ...
    return (
        <Segment clearing>
            ...
            <Formik enableReinitialize initialValues={initialValues} validationSchema={validationSchema} onSubmit={async (values, { setSubmitting }) => {
                ...
                {({ isSubmitting, dirty, isValid, values }) => (
                    <Form className="ui form">
                        ...
                        {selectedEvent && <Button type="button" floated="left" content="Submit" loading={loadingCancel} color={selectedEvent.isCancelled ? 'green' : 'red'} content={selectedEvent.isCancelled ? 'Reactivate event' : 'Cancel event'} onClick={() => setConfirmOpen(true)} />}
            ...
            <Confirm content={selectedEvent?.isCancelled ? 'This will reactivate the event - are you sure?' : 'This will cancel the event - are you sure'} open={confirmOpen} onCancel={() => setConfirmOpen(false)} onConfirm={() => handleCancelToggle(selectedEvent)} />
        </Segment>
    )
}
